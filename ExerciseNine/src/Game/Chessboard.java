...
public class Chessboard {
	...
	public String checkForTwo(char chess)
	{
		...
		for(int i=0;i<3;i++)
		{
			if(board[i][0]==chess_int && board[i][0]==board[i][1])
			{
				return locationToPlace(i, 2);
			}
			if(board[i][0]==chess_int && board[i][0]==board[i][2])
			{
				return locationToPlace(i, 1);
			}
			if(board[i][1]==chess_int && board[i][1]==board[i][2])
			{
		...
		for(int i=0;i<3;i++)
		{
			if(board[0][i]==chess_int && board[0][i]==board[1][i])
			{
				return locationToPlace(2, i);
			}
			if(board[0][i]==chess_int && board[0][i]==board[2][i])
			{
				return locationToPlace(1, i);
			}
			if(board[1][i]==chess_int && board[1][i]==board[2][i])
			{
				return locationToPlace(0, i);
			}
		}
		// Check for two diagonals
		if(board[0][0]==chess_int && board[0][0]==board[1][1])
		{
			return locationToPlace(2, 2);
		}
		if(board[0][0]==chess_int && board[0][0]==board[2][2])
		{
			return locationToPlace(1, 1);
		}
		if(board[1][1]==chess_int && board[2][2]==board[1][1])
		{
			return locationToPlace(0, 0);
		}
		if(board[0][2]==chess_int && board[0][2]==board[1][1])
		{
			return locationToPlace(2, 0);
		}
		if(board[0][2]==chess_int && board[0][2]==board[2][0])
		{
			return locationToPlace(1, 1);
		}
		if(board[1][1]==chess_int && board[0][2]==board[1][1])
		{
			return locationToPlace(2, 0);
		}
		return null;
